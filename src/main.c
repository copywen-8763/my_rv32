...
//         cpu.gpr[rd] = src1 + imm;
//       }
//       printf("[Execute] ADDI x%d = 0x%x + %d -> 0x%x\n", rd, src1, imm, cpu.gpr[rd]);
//     });
...
int main() {
  char buf[128];
  init_mem();
  printf("Type 'q' to quit, 'c' to execute a cycle.\n");
